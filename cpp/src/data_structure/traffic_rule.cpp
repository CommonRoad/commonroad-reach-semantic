...
vector<reach::ReachNodePtr> TrafficRuleInterface::examine_tpl_specifications(
    int const &step, vector<reach::ReachNodePtr> const &vec_nodes_reach,
    ...
    for (auto const &node : vec_nodes_reach) {
        ...
        bool includes_mandatory = std::includes(set_propositions_node.begin(), set_propositions_node.end(),
                                                set_propositions_mandatory.begin(), set_propositions_mandatory.end());

        set<string> intersection{};
        set_intersection(set_propositions_node.begin(), set_propositions_node.end(), set_propositions_forbidden.begin(),
                         set_propositions_forbidden.end(), std::inserter(intersection, intersection.begin()));
