...
InFrontOfObstaclePredicate::InFrontOfObstaclePredicate(bool negated, size_t obstacle_id, double ego_length)
    : CppPredicate(negated, false), obstacle_id(obstacle_id), ego_length(ego_length) {}

std::vector<reach::ReachNodePtr> InFrontOfObstaclePredicate::_restrict_reach_node_mandatory(
...
std::vector<reach::ReachNodePtr> InFrontOfObstaclePredicate::_restrict_reach_node_forbidden(
    int step, const reach::ReachNodePtr &reach_node, const std::shared_ptr<World> &world,
    const std::shared_ptr<CurvilinearCoordinateSystem> &ego_ccs) const {
    auto obstacle_front = _get_obstacle_front(step, world, ego_ccs);
    if (!obstacle_front.has_value()) {
...
std::optional<double>
InFrontOfObstaclePredicate::_get_obstacle_front(int step, const std::shared_ptr<World> &world,
                                                const std::shared_ptr<CurvilinearCoordinateSystem> &ego_ccs) const {
    ...
    return obstacle->frontS(step, ego_ccs) + ego_length / 2.0;
}
