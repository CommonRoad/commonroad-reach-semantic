...
std::vector<reach::ReachNodePtr> KeepSafeDistancePrecPredicate::_restrict_reach_node_mandatory(
    ...
    for (int i = 0; i < NUM_SUPPORT_POINTS; i++) {
        auto safe_position_opt = _determine_safe_position();
        ...
        auto[a, b, c] = _compute_parameters_of_halfspace(step, reach_node, world, ego_ccs, ego_velocity_support);
...
std::vector<reach::ReachNodePtr> KeepSafeDistancePrecPredicate::_restrict_reach_node_forbidden(
    ...
    auto min_ego_velocity = reach_node->v_lon_min();
        ...
        //        reformulated: x - slope(ego_velocity_support) * y <= safe_position(ego_velocity_support)
        auto[a, b, c] =
            _compute_parameters_of_halfspace(step, reach_node, world, ego_ccs, ego_velocity_support);
...
std::tuple<double, double, double> KeepSafeDistancePrecPredicate::_compute_parameters_of_halfspace(
    int step, const reach::ReachNodePtr &reach_node, const shared_ptr<World> &world,
    const shared_ptr<geometry::CurvilinearCoordinateSystem> &ego_ccs, double ego_velocity_support) const {
