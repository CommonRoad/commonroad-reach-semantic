...
BehindObstaclePredicate::BehindObstaclePredicate(bool negated, size_t obstacle_id, double ego_length)
    : CppPredicate(negated, false), obstacle_id(obstacle_id), ego_length(ego_length) {}

std::vector<reach::ReachNodePtr> BehindObstaclePredicate::_restrict_reach_node_mandatory(
    ...
    auto obstacle_behind = _get_obstacle_behind(step, world, ego_ccs);
    if (!obstacle_behind.has_value()) {
...
std::vector<reach::ReachNodePtr> BehindObstaclePredicate::_restrict_reach_node_forbidden(
    int step, const reach::ReachNodePtr &reach_node, const std::shared_ptr<World> &world,
    const std::shared_ptr<CurvilinearCoordinateSystem> &ego_ccs) const {
    auto obstacle_behind = _get_obstacle_behind(step, world, ego_ccs);
    if (!obstacle_behind.has_value()) {
...
std::optional<double>
BehindObstaclePredicate::_get_obstacle_behind(int step, const std::shared_ptr<World> &world,
                                              const std::shared_ptr<CurvilinearCoordinateSystem> &ego_ccs) const {
