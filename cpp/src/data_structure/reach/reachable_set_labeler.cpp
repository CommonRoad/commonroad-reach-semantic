...
ReachableSetLabeler::ReachableSetLabeler(SemanticModelPtr semantic_model, SemanticConfigurationPtr config)
    : semantic_model(std::move(semantic_model)), config(std::move(config)), reachable_set_to_propositions(),
    ...
    for (const auto &reachable_set : reachable_sets) {
        for (const auto &vehicle : semantic_model->vec_vehicles) {
            for (const auto &id_lanelet_propagated_set : reachable_set_to_lanelet_ids[reachable_set]) {
                auto intersecting_lanelets =
                    semantic_model->map_id_lanelet_to_set_ids_lanelets_intersecting[id_lanelet_propagated_set];
                for (const auto &id_lanelet_lane_vehicle : vehicle->lane_lanelet_ids) {
                    if (intersecting_lanelets.find(id_lanelet_lane_vehicle) != intersecting_lanelets.end()) {
                        reachable_set_to_propositions[reachable_set].add_proposition(
                            Proposition::in_conflict_with(vehicle->vehicle_id), PropositionGroup::TRAFFIC_STATUS);
                    }
                }
            }
        }
    ...
    for (const auto &reachable_set : reachable_sets) {
        for (const auto &vehicle : semantic_model->vec_vehicles) {
            ...
            for (const auto &id_lanelet_route : config->semantic_model().vec_route_lanelet_ids) {
                auto intersecting_lanelets =
                    semantic_model->map_id_lanelet_to_set_ids_lanelets_intersecting[id_lanelet_route];
                for (const auto &id_lanelet_vehicle : vehicle->lanelet_ids_at_step(step)) {
                    if (intersecting_lanelets.find(id_lanelet_vehicle) != intersecting_lanelets.end()) {
                        reachable_set_to_propositions[reachable_set].add_proposition(
                            Proposition::in_conflict_by(vehicle->vehicle_id), PropositionGroup::TRAFFIC_STATUS);
                    }
                }
            }
        }
    ...
    for (const auto &reachable_set : reachable_sets) {
        for (const auto &vehicle : semantic_model->vec_vehicles) {
            if (vehicle->braking_caused_by_node_at_step(step, reachable_set)) {
                reachable_set_to_propositions[reachable_set].add_proposition(
                    Proposition::causes_braking_for(vehicle->vehicle_id), PropositionGroup::TRAFFIC_STATUS);
            }
        }
    }
    return reachable_sets;
...
reach::ReachNodePtr ReachableSetLabeler::_update_propositions_with_region(reach::ReachNodePtr propagated_set,
                                                                          const semantic_reach::RegionPtr &region,
                                                                          int step) {
...
ReachableSetLabeler::split_wrt_position_intervals(int step, const std::vector<reach::ReachNodePtr> &reachable_sets) {
    ...
    for (const auto &reachable_set : reachable_sets) {
        ...
        auto vec_nodes_split_lon = _split_reachable_set_wrt_intervals(
            reachable_set, vec_intervals_lon, reachable_set->p_lon_min(), reachable_set->p_lon_max(), "lon");
...
std::vector<reach::ReachNodePtr> ReachableSetLabeler::_split_reachable_set_wrt_intervals(
    const reach::ReachNodePtr &reachable_set, const std::vector<PositionIntervalPtr> &intervals, double reach_min,
...
std::set<std::string>
ReachableSetLabeler::_obtain_lanelet_transition_propositions(const reach::ReachNodePtr &propagated_set) {
    ...
    auto set_propositions_position_source =
        reachable_set_to_propositions[source_node].propositions_in_group(PropositionGroup::POSITION);
