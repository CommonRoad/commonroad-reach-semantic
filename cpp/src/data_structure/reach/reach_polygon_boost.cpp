...
ReachPolygon::ReachPolygon(vector<tuple<double, double>> const& vec_vertices) {
    ...
    for (auto const& vertex: vec_vertices) {
        vec_points_geometry.emplace_back(std::get<0>(vertex), std::get<1>(vertex));
    }
...
ReachPolygon::ReachPolygon(double const& p_lon_min, double const& p_lat_min,
    ...
    vector<GeometryPoint> vec_points_geometry;
    vec_points_geometry.emplace_back(p_lon_min, p_lat_min);
    vec_points_geometry.emplace_back(p_lon_max, p_lat_min);
    vec_points_geometry.emplace_back(p_lon_max, p_lat_max);
    vec_points_geometry.emplace_back(p_lon_min, p_lat_max);
...
ReachPolygon::ReachPolygon(tuple<double, double, double, double> const& tuple_coordinates) {
...
ReachPolygonPtr ReachPolygon::construct_halfspace_polygon(double const& a, double const& b, double const& c,
    ...
    if (b == 0) { // horizontal
        if (a > 0) { // ax <= c
            vec_vertices.emplace_back(c / a, y_min - margin);
...
void ReachPolygon::minkowski_sum(ReachPolygonPtr const& polygon_other) {
    ...
    while (true) {
        ...
        vec_vertices_sum.emplace_back(x_sum, y_sum);
...
bool ReachPolygon::intersects_rectangle(ReachPolygonPtr const& polygon_other, int num_digits) const {
    auto coefficient = pow(10.0, num_digits);

    if (int(p_lon_min() * coefficient) > int(polygon_other->p_lon_max() * coefficient) or
        int(p_lon_max() * coefficient) < int(polygon_other->p_lon_min() * coefficient) or
        int(p_lat_min() * coefficient) > int(polygon_other->p_lat_max() * coefficient) or
        int(p_lat_max() * coefficient) < int(polygon_other->p_lat_min() * coefficient))
        return false;
