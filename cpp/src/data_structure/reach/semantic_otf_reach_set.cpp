...
void SemanticOTFReachableSet::_compute_drivable_area_at_step(const int &step) {
    ...
    // this is done individually for each group calculated above, because we must not merge sets semantically different
    // base sets it is necessary to also consider the states of the propagation source for the partitioning, because
    // only if these are equal, the automaton cannot distinguish the base sets if only the target states were
    // considered, the automaton could possibly distinguish them if the source states reach the target state via
...
void SemanticOTFReachableSet::_compute_reachable_set_at_step(const int &step) {
    ...
    for (auto const &[automaton_states, drivable_area] : map_states_to_drivable_area) {
        ...
        if (discard_small_node) {
            reachable_sets = semantic_reach::discard_nodes_with_short_edge(
                reachable_sets, config->reachable_set().length_edge_node_min);
        }
...
void SemanticOTFReachableSet::_filter_reachable_sets(std::vector<reach::ReachNodePtr> &reachable_sets, int step) {
...
bool SemanticOTFReachableSet::_has_accepting_state(const reach::ReachNodePtr &reachable_set) {
    ...
    return std::any_of(states.begin(), states.end(),
                       [this](const int &state) { return automaton->is_accepting_state(state); });
}
...
SemanticOTFReachableSet::_split_to_minterms(int step, const std::vector<reach::ReachNodePtr> &reachable_sets,
    ...
    std::transform(transitions.begin(), transitions.end(), std::back_inserter(minterms),
                   [](const std::pair<Minterm, unsigned int> &transition) { return transition.first; });
    ...
    if (!literal_to_split_opt) {
        ...
        std::transform(transitions.begin(), transitions.end(), std::inserter(state_labels, state_labels.end()),
                       [](const std::pair<Minterm, unsigned int> &transition) { return transition.second; });
        for (const auto &reachable_set : reachable_sets) {
    ...
    auto [restricted_reachable_sets, restriction_regionized] =
        _restrict_to_literal(step, reachable_sets, literal_to_split, regionized, !not_needs_literal.empty());
    ...
    if (!not_needs_literal.empty()) {
        ...
        auto restricted_reachable_sets_not_needs_literal =
            _split_to_minterms(step, reachable_sets, not_needs_literal, finished_literals, regionized);
        auto restricted_reachable_sets_needs_literal =
            _split_to_minterms(step, restricted_reachable_sets, needs_literal, finished_literals_restricted,
                               regionized || restriction_regionized);
        restricted_reachable_sets_not_needs_literal.insert(
            restricted_reachable_sets_not_needs_literal.end(),
            std::make_move_iterator(restricted_reachable_sets_needs_literal.begin()),
            std::make_move_iterator(restricted_reachable_sets_needs_literal.end()));
...
SemanticOTFReachableSet::_restrict_to_literal(int step, const std::vector<reach::ReachNodePtr> &reachable_sets,
                                              const semantic_reach::Literal &literal, bool regionized, bool clone) {
    ...
    if (pred->needs_lanelets && !regionized) {
        // if the predicate needs lanelets, we need to split the reachable sets into regions first (if we haven't
        // already)
...
SemanticOTFReachableSet::_partition_transitions(const semantic_reach::Literal &literal,
    ...
    std::partition_copy(transitions.begin(), transitions.end(), std::back_inserter(needs_literal),
                        std::back_inserter(not_needs_literal), [&](const auto &transition) {
