...
SemanticModel::SemanticModel(py::handle const &obj_semantic_model_py) {
    ...
    for (auto const &[step, propositions] : dict_step_to_traffic_status_propositions) {
        map_step_to_traffic_status_propositions[step.cast<int>()] =
            dict_step_to_traffic_status_propositions[step].cast<set<string>>();
    }
    ...
    for (auto const &[id_lanelet, set_ids_lanelets_intersecting] : dict_id_lanelet_to_set_ids_lanelets_intersecting) {
        map_id_lanelet_to_set_ids_lanelets_intersecting[id_lanelet.cast<int>()] =
            dict_id_lanelet_to_set_ids_lanelets_intersecting[id_lanelet].cast<set<int>>();
    }
    // vehicles
