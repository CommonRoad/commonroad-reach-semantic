...
private:
    ...
    std::vector<ReachNodePtr> _propagate_reachable_set(std::vector<ReachNodePtr> const& vec_nodes);

    /// Splits propagated sets w.r.t lanelet regions.
    std::vector<ReachNodePtr> _split_wrt_regions(int const& step, std::vector<ReachNodePtr> const& vec_nodes);

    /// Splits the propagated sets w.r.t position intervals.
    std::vector<ReachNodePtr> _split_wrt_intervals(int const& step, std::vector<ReachNodePtr> const& vec_nodes);

    std::vector<ReachNodePtr> _discard_colliding_nodes(std::vector<ReachNodePtr> const& vec_nodes);
    ...
    _compute_collision_free_drivable_area(int const& step,
                                          std::unordered_map<PropositionHolder, std::vector<ReachNodePtr>,
    ...
    std::vector<ReachNodePtr> _call_python_dummy(int const& step, vector<ReachNodePtr> const& vec_nodes);
...
public:
    ...
    std::map<int, std::unordered_map<PropositionHolder, std::vector<ReachNodePtr>, PropositionHolder::HashFunction>>
    ...
    inline std::vector<ReachNodePtr> label_traffic_propositions(int const& step, std::vector<ReachNodePtr> vec_nodes);

    inline ReachNodePtr update_propositions_with_region(ReachNodePtr const& node,
    ...
    inline std::unordered_map<PropositionHolder, std::vector<ReachNodePtr>, PropositionHolder::HashFunction>
    ...
    inline std::vector<ReachNodePtr> reachable_set_merge_at_step(int const& step) {
        if (find(_vec_steps_computed.begin(), _vec_steps_computed.end(), step)
            == _vec_steps_computed.end()) {
            cout << "Given step " << step << "for drivable area retrieval is out of range." << endl;
            return {};
            ...
            std::vector<ReachNodePtr> vec_reachable_set_merged{};
