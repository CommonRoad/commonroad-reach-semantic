...
class SemanticReachableSet {
  ...
  public:
    ...
    inline std::map<int, std::vector<reach::ReachPolygonPtr>> drivable_area() const {
        return map_step_to_drivable_area;
    }
    ...
    inline std::vector<reach::ReachPolygonPtr> drivable_area_at_step(int const &step) {
        if (find(_vec_steps_computed.begin(), _vec_steps_computed.end(), step) == _vec_steps_computed.end()) {
    ...
    inline std::vector<reach::ReachNodePtr> reachable_set_at_step(int const &step) {
        if (find(_vec_steps_computed.begin(), _vec_steps_computed.end(), step) == _vec_steps_computed.end()) {
