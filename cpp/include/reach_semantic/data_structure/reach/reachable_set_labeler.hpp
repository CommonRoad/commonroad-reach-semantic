...
class ReachableSetLabeler {
  private:
    ...
    _obtain_propositions_for_rectangle(const reach::ReachPolygonPtr &rectangle, int step);

    /// Labels propagated sets with traffic status propositions.
    std::vector<reach::ReachNodePtr>
    _label_traffic_status_propositions(int step, std::vector<reach::ReachNodePtr> reachable_sets);
    ...
    reach::ReachNodePtr _update_propositions_with_region(reach::ReachNodePtr propagated_set, const RegionPtr &region,
                                                         int step);

    std::vector<reach::ReachNodePtr>
    _split_reachable_set_wrt_intervals(const reach::ReachNodePtr &reachable_set,
                                       const std::vector<PositionIntervalPtr> &intervals, double reach_min,
  ...
  public:
    ...
    std::vector<reach::ReachNodePtr> label_traffic_propositions(int step,
    ...
    std::vector<reach::ReachNodePtr>
    split_wrt_position_intervals(int step, const std::vector<reach::ReachNodePtr> &reachable_sets);
    ...
    void copy_labels(const reach::ReachNodePtr &source_reachable_set,
                     const std::vector<reach::ReachNodePtr> &reachable_sets);
};
