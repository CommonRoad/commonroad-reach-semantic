...
class SemanticOTFReachableSet : public SemanticReachableSet {
  private:
    ...
    /// in an empty set. The recursion also ends, when we considered all literals. In this case, we label the reachable
    /// sets with the target states of the transitions that they satisfy.
    /// @param step Current step of the reachability analysis.
    ...
    /// @return List of reachable sets so that each is a subset of the given reachable sets, and satisfies the condition
    /// of at least one transition (up to overapproximation).
    std::vector<reach::ReachNodePtr>
    ...
    /// If restricting requires lanelet information, we first split the reachable sets into regions (if we haven't
    /// already). If we clone the reachable sets, we also copy the labels from the original nodes to the clones.
    /// @param step Current step of the reachability analysis.
    ...
    /// @return A tuple of two dictionaries, the first containing the transitions that don't depend on the literal, the
    /// second containing the transitions that do.
    ...
    std::optional<Literal> _choose_next_literal(const std::vector<Minterm> &minterms,
