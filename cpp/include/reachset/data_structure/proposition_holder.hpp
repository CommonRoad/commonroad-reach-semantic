...
struct PropositionHolder {
    ...
    bool operator==(PropositionHolder const& other) const {
        return set_propositions == other.set_propositions;
    }
    ...
    inline void add_propositions(std::set<std::string> const& set_propositions, py::handle const& group_py) {
        for (auto const& proposition: set_propositions) {
            add_proposition(proposition,
                            get_proposition_group(group_py.attr("name").cast<std::string>()));
        }
    }
    ...
    inline std::set<std::string> propositions(bool include_temporary = true) const {
        if (include_temporary) {
            ...
            std::set_union(set_propositions.begin(), set_propositions.end(),
                           set_propositions_temporary.begin(), set_propositions_temporary.end(),
                           std::inserter(set_output, set_output.begin()));
    ...
    inline std::set<std::string> propositions_in_group(py::handle const& group_py) {
        return map_group_to_set_propositions[get_proposition_group(group_py.attr("name")
                                                                           .cast<std::string>())];
    }

    static inline PropositionGroup get_proposition_group(std::string const& group_name) {
        if (group_name == "POSITION") return PropositionGroup::POSITION;
        else if (group_name == "VELOCITY") return PropositionGroup::VELOCITY;
        else if (group_name == "ACCELERATION") return PropositionGroup::ACCELERATION;
        else if (group_name == "VEHICLE") return PropositionGroup::VEHICLE;
        else if (group_name == "TRAFFIC_SIGN") return PropositionGroup::TRAFFIC_SIGN;
        else if (group_name == "TRAFFIC_LIGHT") return PropositionGroup::TRAFFIC_LIGHT;
        else if (group_name == "INTERSECTION") return PropositionGroup::INTERSECTION;
        else if (group_name == "PRIORITY") return PropositionGroup::PRIORITY;
        else if (group_name == "TRAFFIC_STATUS") return PropositionGroup::TRAFFIC_STATUS;
        else if (group_name == "TEMPORARY") return PropositionGroup::TEMPORARY;
        else {
...
struct MultiStepPropositionHolder {
    ...
    inline auto time_invariant_propositions() const {
        return set_propositions_time_invariant;
    }
    ...
    inline auto time_variant_propositions() const {
        return map_step_to_set_propositions_time_variant;
    }
    ...
    inline auto propositions_at_step(int const& step) {
        ...
        std::set_union(set_propositions_time_invariant.begin(),
                       set_propositions_time_invariant.end(),
    ...
    inline auto map_group_to_propositions_at_step(int const& step) {
        ...
        for (auto const& [group, set_propositions]:
                map_group_to_set_propositions_time_invariant) {
        ...
        for (auto& [group, map_step_to_set_propositions]:
                map_group_to_step_to_set_propositions_time_variant) {
    ...
    inline auto add_propositions(std::string const& proposition,
                                 PropositionGroup const& group, int const& step = -1) {
        if (step == -1) {
