#include "test_behind_obstacle_predicate.hpp"
...
TEST_F(BehindObstaclePredicateTest, Positive) {
    ...
    // expect lon max set to obstacle rear inflated by ego length
...
TEST_F(BehindObstaclePredicateTest, Negative) {
    auto restricted = negative_pred->restrict_reach_node(TestEnvironments::time_step, reach_node_one, {},
                                                         test_envs.world_one, test_envs.ccs_one);
    // Splitting is not necessary for this predicate
    ASSERT_EQ(restricted.size(), 1);
    auto restricted_node = restricted.at(0);
    // expect lon min set to obstacle rear inflated by ego length
