...
void export_reachable_set_interface(py::module& m) {
    py::class_<ReachableSet, shared_ptr<ReachableSet>>(m, "ReachableSet")
            ...
            .def_readonly("map_step_to_propositions_to_drivable_area",
                          &ReachableSet::map_step_to_propositions_to_drivable_area)
            .def_readonly("map_step_to_propositions_to_reachable_set",
                          &ReachableSet::map_step_to_propositions_to_reachable_set)
                    //.def_readwrite("map_time_to_reachable_set_refined",
                    //               &ReachableSet::map_time_to_reachable_set_refined)
            .def_readonly("config", &ReachableSet::config)
                    //.def("print_collision_checker_info", &ReachableSet::print_collision_checker_info)
            .def("compute", &ReachableSet::compute,
                 py::arg("step_start") = 1,
                 py::arg("step_end") = 0)
            .def("drivable_area_at_step", &ReachableSet::drivable_area_at_step,
                 py::arg("step"))
            .def("drivable_area_merge_at_step", &ReachableSet::drivable_area_merge_at_step,
                 py::arg("step"))
            .def("reachable_set_at_step", &ReachableSet::reachable_set_at_step,
                 py::arg("step"))
            .def("reachable_set_merge_at_step", &ReachableSet::reachable_set_merge_at_step,
                 py::arg("step"));
    //.def("refined_reachable_set_at_time_step", &ReachableSet::refined_reachable_set_at_time_step,
    //     py::arg("time_step"))
    //.def("add_refined_node", &ReachableSet::add_refined_node, py::arg("time_step"), py::arg("node"));
}