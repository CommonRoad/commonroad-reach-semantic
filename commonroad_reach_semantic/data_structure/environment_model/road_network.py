...
from commonroad_route_planner.utility.polyline_operations.polyline_operations import resample_polyline
...
class Lane:
    ...
    def __init__(self, lanelet_merged: Lanelet, list_ids_lanelets_merged: List[int]):
        ...
        self._list_ids_lanelets_merged = list_ids_lanelets_merged
    ...
    def __repr__(self):
        return f"Lane(lanelet_ids={self._list_ids_lanelets_merged})"
    ...
    def list_ids_lanelets(self) -> List[int]:
        return self._list_ids_lanelets_merged
    ...
    def create_curvilinear_coordinate_system_from_reference(ref_path: np.array) \
        ...
        curvilinear_cosy = CurvilinearCoordinateSystem(ref_path_new, 50.0, 0.1)
        return curvilinear_cosy


class RoadNetwork:
    """
    Class representing the complete road network of a CommonRoad scenario abstracted to lanes.
    """

    def __init__(self, lanelet_network: LaneletNetwork, length_merge_lanelets: int = 150):
        self.lanelet_network = lanelet_network
        self.list_lanes = self._create_lanes(length_merge_lanelets)

    def _create_lanes(self, length_merge_lanelets: int) -> List[Lane]:
        """
        Creates lanes for the road network.
        """
        list_lanes = []
        list_tuples_result = []

        # obtain all lanelets with no predecessor or with a predecessor but has a different lanelet type
        set_lanelets_start = set()
        for lanelet in self.lanelet_network.lanelets:
            if not len(lanelet.predecessor):
                set_lanelets_start.add(lanelet)

            else:
                list_lanelets_pre = \
                    [self.lanelet_network.find_lanelet_by_id(id_pred) for id_pred in lanelet.predecessor]
                for lanelet_pre in list_lanelets_pre:
                    if lanelet.lanelet_type != lanelet_pre.lanelet_type:
                        set_lanelets_start.add(lanelet)

        # create all possible merged lanes starting from each lanelet in the start set
        for lanelet_start in set_lanelets_start:
            list_lanelets_merged, list_ids_lanelets_merged = \
                Lanelet.all_lanelets_by_merging_successors_from_lanelet(lanelet_start, self.lanelet_network,
                                                                        length_merge_lanelets)
            # if there is no successor to be merged, add the start lanelet itself
            if not len(list_lanelets_merged) or not len(list_ids_lanelets_merged):
                list_lanelets_merged.append(lanelet_start)
                list_ids_lanelets_merged.append([lanelet_start.lanelet_id])
    ...
    def find_lane_ids_by_lanelets(self, lanelets: Set[int]) -> Set[int]:
        ...
        for lane in self.list_lanes:
            for lanelet_id in lanelets:
                if lanelet_id in lane.list_ids_lanelets:
                    lane_ids.add(lane.lanelet.lanelet_id)

        return lane_ids

    def find_lanes_by_lanelets(self, set_ids_lanelets: Set[int]) -> Set[Lane]:
        """
        ...
        for lane in self.list_lanes:
            for id_lanelet in set_ids_lanelets:
                if id_lanelet in lane.list_ids_lanelets:
                    lanes.add(lane)

        return lanes

    def find_lane_by_lanelet(self, lanelet_id: int) -> Set[Lane]:
        """
        Finds the lanes a lanelet belongs to.
        """
        set_lanes = set()
        for lane in self.list_lanes:
            if lanelet_id in lane.list_ids_lanelets:
    ...
    def find_lane_by_obstacle(self, obs_lanelet_center: List[int], obs_lanelet_shape: List[int]) -> Lane:
        ...
        if len(obs_lanelet_center) > 0:
            for lane in self.list_lanes:
                for lanelet in lanelets_center_updated:
                    if lanelet in lane.list_ids_lanelets:
        ...
        for lane in occupied_lanes:
            for lanelet_id in lane.list_ids_lanelets:
