...
class VehicleInConflictAreaPredicate(predicate.Predicate):
    ...
    def _restrict_reach_node_mandatory(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
        ...
        if p_lon_max := self._p_lon_max_for_conflict_at_step(step, semantic_model):
            ...
            # if we couldn't determine the maximal longitudinal position,
            # the vehicle must be too far away to convert into the CLCS,
            # so we assume that there is no conflict
            return []
    ...
    def _p_lon_max_for_conflict_at_step(self, step: int, semantic_model: SemanticModel) -> Optional[float]:
