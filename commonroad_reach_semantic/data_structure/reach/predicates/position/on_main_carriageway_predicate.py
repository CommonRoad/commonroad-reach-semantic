...
class OnMainCarriagewayPredicate(predicate.Predicate):
    ...
    def _restrict_reach_node_mandatory(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
        ...
        if not node_lanelet_ids.isdisjoint(semantic_model.lanelet_model.main_carriageway_lanelet_ids):
            return [reach_node]
        else:
            return []
    ...
    def _restrict_reach_node_forbidden(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
                                       node_lanelet_ids: Set[int]) -> List[ReachNode]:
        if node_lanelet_ids.isdisjoint(semantic_model.lanelet_model.main_carriageway_lanelet_ids):
            return [reach_node]
        else:
            return []
