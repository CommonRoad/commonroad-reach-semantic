...
class HasPriorityPredicate(predicate.Predicate):
    ...
    def to_proposition(self) -> str:
        return Prop.has_priority(self.vehicle_id, self.direction_ego, self.direction_other)
    ...
    def _restrict_reach_node_mandatory(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
        ...
        return [reach_node] if self._ego_has_priority(semantic_model, node_lanelet_ids, step) else []

    @predicate.needs_lanelets_set
    def _restrict_reach_node_forbidden(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
                                       node_lanelet_ids: Set[int]) -> List[ReachNode]:
        return [reach_node] if not self._ego_has_priority(semantic_model, node_lanelet_ids, step) else []

    def _ego_has_priority(self, semantic_model, node_lanelet_ids, step):
        """Check if the ego vehicle has priority over the other vehicle for the indicated directions at the given step.

        The ego vehicle has priority if any of its occupied lanelets has priority over all occupied lanelets of the other vehicle.
