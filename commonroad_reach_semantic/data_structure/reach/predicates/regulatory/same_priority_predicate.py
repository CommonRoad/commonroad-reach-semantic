...
class SamePriorityPredicate(predicate.Predicate):
    ...
    def _restrict_reach_node_mandatory(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
        ...
        return [reach_node] if self._is_same_priority(semantic_model, node_lanelet_ids, step) else []

    @predicate.needs_lanelets_set
    def _restrict_reach_node_forbidden(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
                                       node_lanelet_ids: Set[int]) -> List[ReachNode]:
        return [reach_node] if not self._is_same_priority(semantic_model, node_lanelet_ids, step) else []

    def _is_same_priority(self, semantic_model, node_lanelet_ids, step):
        """Check if the ego vehicle and the other vehicle have the same priority for the indicated directions at the given step.

        The vehicles have the same priority if neither vehicle has priority over the other.
