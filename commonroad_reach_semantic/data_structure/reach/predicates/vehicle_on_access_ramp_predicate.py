...
class VehicleOnAccessRampPredicate(predicate.Predicate):
    def __init__(self, vehicle_id: int, negated: bool):
        super().__init__(negated)
        self.vehicle_id = vehicle_id

    def to_proposition(self) -> str:
        return Prop.vehicle_on_access_ramp(self.vehicle_id)
    ...
    def _restrict_reach_node_mandatory(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
        ...
        return [reach_node] if self._is_vehicle_on_access_ramp(step, semantic_model) else []

    def _restrict_reach_node_forbidden(self, step: int, reach_node: ReachNode, semantic_model: SemanticModel,
                                       node_lanelet_ids: Optional[Set[int]] = None) -> List[ReachNode]:
        return [reach_node] if not self._is_vehicle_on_access_ramp(step, semantic_model) else []

    def _is_vehicle_on_access_ramp(self, step: int, semantic_model: SemanticModel) -> bool:
        if vehicle := semantic_model.vehicle_model.find_vehicle_by_id(self.vehicle_id):
            return not semantic_model.lanelet_model.access_ramp_lanelet_ids.isdisjoint(
